...
void synthesize(
    ...
    if(profbeta){
      ...
      (*ll)(0) = -0.5*(logdet + 2.0*logdet2 + (n-p)*std::log(n*sig2) );
    }else{
      (*ll)(0) = -0.5*(logdet + n*std::log(n*sig2) );
    }
    ...
    for(int j=0; j<nparms; j++){
      ...
      if(profbeta){
        (*grad)(j) += 0.5*dlogdet2(j);
        double dsig2 = 0;
        dsig2 += 0.5*dySy(j);
        dsig2 -= 1.0*as_scalar( abeta.t() * dySX.col(j) );
        dsig2 += 1.0*as_scalar( ySX.t() * dbeta.col(j) );
        dsig2 += 0.5*as_scalar( abeta.t() * dXSX.slice(j) * abeta );
        dsig2 -= 1.0*as_scalar( abeta.t() * XSX * dbeta.col(j) );
