          ...
          if(method=='post_mode'){
            if(prior_choice=='ref_approx'){####this one can be with nugget or without the nugget
              ...
              if(vecchia){
                ...
                # ord = GpGp4RG::order_maxmin(design)
                # design.ord = design[ord,]
                # X.ord = model@X[ord,]
                # y.ord = model@output[ord,,drop=F]
                # NNarray = GpGp4RG::find_ordered_nn(design.ord, m)
                #
                ...
                ord = GpGp4RG::order_maxmin(t(t(design)*scales))
                design.ord = design[ord,,drop=F]
                y.ord = model@output[ord,,drop=F]
                X.ord = model@X[ord,,drop=F]
                NNarray = GpGp4RG::find_ordered_nn(t(t(design.ord)*scales), m)
                ...
                # # NNarray = NA
                # 
                # ord = GpGp4RG::order_maxmin(design_subset)
