...
predict.ppgasp <- function (object, testing_input, testing_trend= matrix(1,dim(testing_input)[1],1), 
  ...
  m_pred = as.integer(m_pred)
  if(object@NN_predict){
    testing_input=as.matrix(testing_input)
    ...
    output.mean = matrix(0,nrow(testing_input),ncol(object@output))
    output.upper95 = matrix(0,nrow(testing_input),ncol(object@output))
    ...
    for(i_n_pred in 1:nrow(testing_input)){
      ...
      model <- save_object
      current_output = save_object@output[NN_ind,,drop=F]
      current_R0 = lapply(save_object@R0, function(mat) mat[NN_ind,NN_ind,drop=F])
      current_X = save_object@X[NN_ind,,drop=F]
      list_return=construct_ppgasp(model@beta_hat, model@nugget, current_R0, current_X, 
                                   zero_mean=model@zero_mean,
                                   current_output,kernel_type_num,model@alpha);
      model@output = current_output
      model@R0 = current_R0
      model@input = save_object@input[NN_ind,,drop=F]
      model@num_obs = nrow(model@input)
      model@X = current_X
      ...
      model@k = length(model@sigma2_hat);
      
      temp_object <- model
      ####let users specify the trend
      num_testing_input <- dim(testing_input_i)[1]
      #X_testing = matrix(1,num_testing_input,1) ###testing trend
      
      qt_025=qt(0.025,df=(temp_object@num_obs-temp_object@q))
      qt_975=qt(0.975,df=(temp_object@num_obs-temp_object@q))
      
      
      testing_input_i=as.matrix(testing_input_i)
      
      ##form the r0 matrix
      p_x <- dim(temp_object@input)[2]
      
      if(is.na(r0)[1]){
        if(!temp_object@isotropic){
          r0 = as.list(1:temp_object@p)
          for(i in 1:temp_object@p){
            r0[[i]] = as.matrix(abs(outer(testing_input_i[,i], temp_object@input[,i], "-")))
          }
        }else{
          r0 = as.list(1)
          if(p_x<temp_object@num_obs){
            r0_here=0
            for(i in 1:p_x){
              r0_here=r0_here+(as.matrix(abs(outer(testing_input_i[,i], temp_object@input[,i], "-"))))^2
            }
            r0[[1]]=sqrt(r0_here)
          }else{
            r0[[1]]=euclidean_distance(testing_input_i,temp_object@input)
          }
        }
      }else if(class(r0)[1]=='matrix'){
        r0_here=r0
      ...
      if(length(r0)!=temp_object@p){
        stop("the number of R0 matrices should be the same 
         as the number of range parameters in the kernel \n")
      }
      if( (dim(r0[[1]])[1]!=num_testing_input) | (dim(r0[[1]])[2]!=temp_object@num_obs)){
        stop("the dimension of R0 matrices should match the number of observations \n")
      }
      
      
      ##change kernel type to integer to pass to C++ code
      kernel_type_num=rep(0,  temp_object@p)
      for(i_p in 1:  temp_object@p){
        if(temp_object@kernel_type[i_p]=="matern_5_2"){
          kernel_type_num[i_p]=as.integer(3)
        }else if (temp_object@kernel_type[i_p]=="matern_3_2"){
          kernel_type_num[i_p]=as.integer(2)
        }else if (temp_object@kernel_type[i_p]=="pow_exp"){
          kernel_type_num[i_p]=as.integer(1)
        }else if (temp_object@kernel_type[i_p]=="periodic_gauss"){  ##this is periodic folding on Gaussian kernel
          kernel_type_num[i_p]=as.integer(4)
        }else if (temp_object@kernel_type[i_p]=="periodic_exp"){   ##this is periodic folding on Exponential kernel
          kernel_type_num[i_p]=as.integer(5)
        }
      }
      
      
      #pred_list=pred_ppgasp(temp_object@beta_hat,temp_object@nugget,temp_object@input,temp_object@X,temp_object@zero_mean,temp_object@output,
      #                     testing_input_i,testing_input_i,temp_object@L,temp_object@LX,temp_object@theta_hat,
      #                     temp_object@sigma2_hat,qt_025,qt_975,r0,kernel_type_num,temp_object@alpha)
      if(is.na(loc_index[1])){
        pred_loc_index=1:dim(temp_object@output)[2]
      }else{
        pred_loc_index=loc_index
        if(min(loc_index)<1 |max(loc_index)>dim(temp_object@output)[2] ){
          stop("loc_index should not be smaler than 1 or larger than the number of columns of output \n")
        }
      }
      if(dim(temp_object@theta_hat)[1]==1){
        theta_hat_pred_loc=t(as.matrix(temp_object@theta_hat[,pred_loc_index]))
      }else{
        theta_hat_pred_loc=as.matrix(temp_object@theta_hat[,pred_loc_index])
      }
      
      if((temp_object@method=='post_mode')|(temp_object@method=='mmle')){
        qt_025=qt(0.025,df=(temp_object@num_obs-temp_object@q))
        qt_975=qt(0.975,df=(temp_object@num_obs-temp_object@q))
        pred_list=pred_ppgasp(temp_object@beta_hat,temp_object@nugget,temp_object@input,temp_object@X,temp_object@zero_mean,as.matrix(temp_object@output[,pred_loc_index]),
                              testing_input_i,testing_trend_i,temp_object@L,temp_object@LX,theta_hat_pred_loc,
                              temp_object@sigma2_hat[pred_loc_index],qt_025,qt_975,r0,kernel_type_num,temp_object@alpha,temp_object@method,interval_data)
      }else if(temp_object@method=='mle'){
        qn_025=qnorm(0.025,mean=0,sd=1)
        qn_975=qnorm(0.975,mean=0,sd=1)
        pred_list=pred_ppgasp(temp_object@beta_hat,temp_object@nugget,temp_object@input,temp_object@X,temp_object@zero_mean,as.matrix(temp_object@output[,pred_loc_index]),
                              testing_input_i,testing_trend_i,temp_object@L,temp_object@LX,theta_hat_pred_loc,
      ...
      output.sd[i_n_pred,] = sqrt(pred_list[[4]])
      ...
      #   output.list$sd=sqrt(pred_list[[4]])
      # }else{
      #   output.list$mean = rbind(output.list$mean, pred_list[[1]])
      #   output.list$lower95 = rbind(output.list$lower95, pred_list[[2]])
      #   output.list$upper95 = rbind(output.list$upper95, pred_list[[3]])
      #   output.list$sd = rbind(output.list$sd, sqrt(pred_list[[4]]))
      # }
    }
    output.list <- list()
    output.list$mean = output.mean
    output.list$lower95 = output.lower95
    output.list$upper95 = output.upper95
    output.list$sd = output.sd
  }else{
    if(!is.na(m_pred)){
      m_pred = NA_integer_
      cat('Because NN_predict was FALSE, m_pred has been redefined as NA.')
    }
    testing_input=as.matrix(testing_input)
